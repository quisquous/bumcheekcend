...
string consultCasual(int round, string opp, string text) {
	...
	//print(opp);
	if ($strings[Ed the Undying, ancient protector spirit, protector spirit, gaudy pirate, modern zmobie, conjoined zmombie, gargantulihc, huge ghuol, giant skeelton, dirty old lihc, swarm of ghuol whelps, The Bonerdagon] contains opp) {
		...
		if ($strings[dirty old lihc] contains opp) {
	...
	//Monsters to abort on. I can't be bothered to automate this yet. 
	...
	print("BCC: Run away, Brave Sir Robin!", "purple");
	return "runaway";
}
...
boolean bcascAirship() {
	...
	if (!in_hardcore()) airshipGoals = "1 S.O.C.K.";
...
boolean bcascChasm() {
	...
	while (contains_text(visit_url("questlog.php?which=1"), "A Quest, LOL")) {
		if (index_of(visit_url("questlog.php?which=1"), "to the Valley beyond the Orc Chasm") > 0) {
			...
			if (i_a("64735 scroll") == 0 || (i_a("lowercase N") == 0 && in_hardcore())) bumAdv($location[Orc Chasm], "", "items", "1 64735 scroll, 1 lowercase N", "Get me the 64735 Scroll", "i");
...
void bcascDailyDungeon() {
	...
	while (!contains_text(visit_url("dungeon.php"), "reached the bottom")) cli_execute("adv " + my_adventures() + " daily dungeon");
	zapKeys();
}
...
boolean bcascFriarsSteel() {
	...
	if (can_interact()) return checkStage("friarssteel", true);
...
boolean bcascGuild1() {
	if (!in_hardcore()) return checkStage("guild1", true);
...
boolean bcascGuild2() {
	...
	if (!in_hardcore()) return checkStage("guild2", true);
					...
					switch (lairitems[x].a) {
						case "marzipan skull" :
							if (gnomads_available()) {
								...
								tryPull(lairitems[x].a, true);
							}
						break;
						
						case "Meleegra pills" :
							while (item_amount($item[Meleegra pills]) == 0) {
								if (!tryPull(lairitems[x].a)) bumAdv($location[South of the Border], "", "items", "1 Meleegra pills", "Getting some Meelegra pills", "-i");
							}
						break;
						
						case " gremlin juice" :
							if (!tryPull(lairitems[x].a)) bumAdv($location[post-war junkyard], "", "hebo", "1 gremlin juice", "Getting gremlin juice", "", "consultHeBo");
						break; 
	...
	while (contains_text(visit_url("questlog.php?which=1"), "Gotta Worship Them All")) {
		...
		switch (my_primestat()) {
			...
			case $stat[Moxie] :
				...
				if (in_hardcore()) abort("Due to the Noodles attack, it is hard to automate this fight. You must do this manually.");
			break;
		}
	...
	boolean doSideQuest(string name) {
		...
		switch (name) {
			...
			case "nuns" :
				...
				buMax("+outfit "+bcasc_warOutfit+" "+prepSNS());
