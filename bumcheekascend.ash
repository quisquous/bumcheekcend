...
if (index_of(visit_url("http://kolmafia.us/showthread.php?t=4963"), "0.13</b>") == -1) {
...
boolean identifyBangPotions() {
	...
	//Returns true if there are some unknown potions that we should find out about by throwing them against monsters. 
	boolean somePotionsUnknown() {
		foreach pot, eff in allBangPotions() {
			if (eff == $effect[none]) return true;
		}
		return false;
	}
	
	while (numPotionsFound() < 5 && somePotionsUnknown()) {
		adventure(1, $location[Hole in the Sky], "consultDoD");
	}
...
boolean setFamiliar(string famtype) {
	...
	if (i_a("spangly sombrero") > 0 && have_familiar($familiar[Mad Hatrack]) && contains_text(famtype, "item")) {
	...
	if (have_familiar($familiar[Rogue Program]) || have_familiar($familiar[Baby Sandworm])) {
		...
		if (my_spleen_use() < 12 && my_level() >= 4) {
			print("BCC: Going to try to use some spleen items if you have them.", "purple");
			
			while (my_spleen_use() < 12 && item_amount($item[agua de vida]) > 0) {
				use(1, $item[agua de vida]);
			}
			
			visit_url("town_wrong.php");
			while (my_spleen_use() < 12 && (available_amount($item[coffee pixie stick]) > 0 || item_amount($item[Game Grid token]) > 0)) {
		...
		if (my_spleen_use() + (i_a("agua de vida") + i_a("coffee pixie stick") + i_a("Game Grid token") + i_a("Game Grid ticket")/10) * 4 < 12) {
...
string tryBeerPong() {
	...
	if (contains_text(page, "Combat")) {
		...
		while(!page.contains_text("You win the fight!")) page = run_combat();
...
void setMood(string combat) {
	...
	if (contains_text(combat,"+")) {
		if (my_level() >= 9 || willMood()) {
			...
			cli_execute("trigger lose_effect, Musk of the Moose, cast 1 Musk of the Moose");
			cli_execute("trigger lose_effect, Carlweather's Cantata of Confrontation, cast 1 Carlweather's Cantata of Confrontation");
	...
	if (contains_text(combat,"-")) {
		if (my_level() >= 9 || willMood()) {
			...
			cli_execute("trigger lose_effect, The Sonata of Sneakiness, cast 1 sonata of sneakiness");
	...
	if (contains_text(combat,"i")) {
		if (my_level() >= 9 || willMood()) {
			...
			cli_execute("trigger lose_effect, Fat Leon's Phat Loot Lyric, cast 1 Fat Leon's Phat Loot Lyric");
...
boolean levelMe(int sMox, boolean needBaseStat) {
	...
	cli_execute("goal clear; goal set "+sMox+" Moxie");
	...
	if (my_buffedstat($stat[Moxie]) < 90)
	{
		...
		cli_execute("maximize mox +outfit swash -melee");
		adventure(my_adventures(), $location[Barrrney's Barrr]);
	} else {
		setMood("-i");
		setFamiliar("itemsnc");
		...
		if (my_level() >= 10) {
			if (item_amount($item[ten-leaf clover]) + item_amount($item[disassembled clover]) > 0) {
		...
		while (my_basestat($stat[Moxie]) < sMox) {
			...
			if (item_amount($item[dance card]) > 0) {
				...
				cli_execute("goal set 1 dance card");
...
boolean bumAdv(location loc, string maxme, string famtype, string goals, string printme, string combat, string consultScript) {
	...
	if ((consultScript == "consultHeBo") && (my_familiar() != $familiar[He-Boulder])) {
	...
	if (maxme == "") {
		maxme = "moxie -tie -melee -ml";
	}
	cli_execute("maximize "+maxme+" -0.01 ml");
	
	if (my_buffedstat($stat[Moxie]) < sMox && loc != $location[Haunted Bedroom])	{
		//Do something to get more moxie.
		print("Need to Level up a bit to get "+sMox+" Moxie");
		levelMe(sMox);
	}
	cli_execute("mood execute");
	...
	//Force to 4 in Boss Bat, 7 in Knob King
	if (canMCD() && loc == $location[Boss Bat's Lair]) { cli_execute("mcd 4"); }
	if (canMCD() && loc == $location[King's Chamber]) { cli_execute("mcd 7"); }
...
boolean bcasc8Bit() {
	...
	//Because of the Transfunctionator, we'd best leave this until now. 
	while (i_a("digital key") == 0) {
		...
		bumAdv($location[8-Bit Realm], "mox +equip continuum transfunctioner, 1 hand, -melee, -tie", "items", "1 digital key", "Getting the Digital Key");
	}
	...
	if (!contains_text(visit_url("questlog.php?which=2"), "slain the Boss Bat")) {
		...
		while (item_amount($item[sonar-in-a-biscuit]) < 1 && contains_text(visit_url("bathole.php"), "action=rubble")) {
			...
			if (i_a("sonar-in-a-biscuit") == 0 && cloversAvailable(true) > 0) {
				...
				bumAdv($location[Guano Junction], "mox -melee, -ML, +10stench res", "items", "1 sonar-in-a-biscuit", "Getting a Sonars");
			}
...
boolean bcascCyrpt() {
	...
	while (index_of(visit_url("cyrpt.php"), "cyrpt7d.gif") > 0) bumAdv($location[Defiled Nook], "", "", "", "Un-Defiling the Nook (1/4)", "-");
	while (index_of(visit_url("cyrpt.php"), "cyrpt4d.gif") > 0) bumAdv($location[Defiled Alcove], "", "", "", "Un-Defiling the Alcove (2/4)", "-");
	while (index_of(visit_url("cyrpt.php"), "cyrpt6d.gif") > 0) bumAdv($location[Defiled Niche], "", "", "", "Un-Defiling the Niche (3/4)", "-");
...
boolean bcascEpicWeapons() {
	...
	if (my_basestat(my_primestat()) > 10 && i_a("Rock and Roll Legend") == 0 && i_a("Squeezebox of the Ages") == 0 && i_a("The Trickster's Trikitixa") == 0) {
		print("BCC: Getting the AT Epic Weapon", "purple");
		
		while (i_a("stolen accordion") == 0) use(1, $item[chewing gum on a string]);
		if (i_a("hot buttered roll") == 0) cli_execute("hermit hot buttered roll");
		...
		if (my_meat() < 1000 && i_a("tenderizing hammer") == 0) return false;
		cli_execute("make rock and roll legend");
	}
	...
	if (my_class() == $class[Disco Bandit] && my_basestat(my_primestat()) > 10 && i_a("Disco Banjo") == 0 && i_a("Shagadelic Disco Banjo") == 0 && i_a("Seeger's Unstoppable Banjo") == 0) {
		print("BCC: Getting the DB Epic Weapon", "purple");
		
		while (i_a("disco ball") == 0) use(1, $item[chewing gum on a string]);
		if (i_a("banjo strings") == 0) cli_execute("hermit banjo strings");
		...
		if (my_meat() < 1000 && i_a("tenderizing hammer") == 0) return false;
		if (cli_execute("make disco banjo")) {}
	}
}
...
boolean bcascFriarsSteel() {
	...
	string steelName = "steel margarita", whatToDo = "drink steel margarita";
	int limitToCheckFor = inebriety_limit();
	if (!can_drink()) {
		if (!can_eat()) {
	...
	while (limitToCheckFor < 19) {
		if (i_a(steelName) > 0) {
			print("BCC: Drinking "+steelName, "purple");
			if (cli_execute(whatToDo)) {}
			if (i_a(steelName) > 0) { return false; }
		} else {
...
boolean bcascGuild2() {
	...
	if (index_of(visit_url("guild.php?place=challenge"), "you to steal his wallet") > 0) {
		if (my_buffedstat(my_primestat()) > 20) {
			print("BCC: Doing the Second Challenge...", "purple");
			if (my_adventures() == 0) abort("You are out of adventures.");
			visit_url("guild.php?action=chal");
			visit_url("guild.php?place=paco");
...
boolean bcascHoleInTheSky() {
	...
	if (i_a("star hat") == 0 || i_a("star crossbow") == 0 || i_a("richard's star key") == 0) {
		(!retrieve_item(1, $item[star hat]));
		while (i_a("star hat") == 0) {
			bumAdv($location[Hole in the Sky], "", "items", "1 star hat", "Getting the Star Hat", "i");
			(!retrieve_item(1, $item[star hat]));
		}
		(!retrieve_item(1, $item[star crossbow]));
		while (i_a("star crossbow") == 0) {
			bumAdv($location[Hole in the Sky], "", "items", "1 star crossbow", "Getting the Star Crossbow", "i");
			(!retrieve_item(1, $item[star crossbow]));
		}
		(!retrieve_item(1, $item[richards star key]));
		while (i_a("Richards star key") == 0) {
			bumAdv($location[Hole in the Sky], "", "items", "1 richards star key", "Getting the Star Key", "i");
			(!retrieve_item(1, $item[richards star key]));
		}
	}
...
boolean bcascKnobKing() {
	...
	if (!contains_text(visit_url("questlog.php?which=2"), "slain the Goblin King")) {
		...
		cli_execute("maximize mox -melee");
		if (my_buffedstat($stat[Moxie]) >= 75) {
			if (canMCD()) cli_execute("mcd 7");
			bumAdv($location[King's Chamber], "mox +outfit harem", "", "", "Killing the Knob King");
...
boolean bcascMacguffinHiddenCity() {
	...
	while (i_a("spectre scepter") == 0 && (item_amount($item[ancient amulet]) + item_amount($item[staff of ed]) == 0)) {
		...
		cli_execute("maximize moxie +outfit hippy -tie -ML -melee");
...
boolean bcascMacguffinPalindome() {
	...
	while (contains_text(visit_url("questlog.php?which=1"), "Never Odd")) {
		while (contains_text(visit_url("questlog.php?which=1"), "Palindome") && item_amount(to_item("i love me")) == 0)
			bumAdv($location[Palindome], "mox -ml, -melee, +equip talisman o' nam", "hebo", "1 stunt nuts, 1 I Love Me Vol I", "Getting the 'I Love Me' from the Palindome", "", "consultHeBo"); 
		...
		//Fight Dr. Awkward
		cli_execute("restore hp; conditions clear; maximize mox -tie -melee -0.01 ml +equip Talisman o' Nam +equip Mega Gem");
...
boolean bcascMacguffinPrelim() {
	...
	if (i_a("Talisman o' Nam") == 0 && my_basestat($stat[Mysticality]) >= 60) {
		...
		if (!contains_text(visit_url("cove.php"), "cove3_5x2b.gif")) bumAdv($location[Poop Deck], "mox +equip pirate fledges", "", "", "Opening Belowdecks");
		bumAdv($location[Belowdecks], "mox +equip pirate fledges", "", "2 gaudy key", "Getting the Talisman");
		if (cli_execute("use 2 gaudy key")) {}
		if (cli_execute("make talisman o nam")) {}
	}
...
boolean bcascMacguffinSpooky() {
	...
	if (contains_text(visit_url("questlog.php?which=1"),"Spooking")) {
		...
		if (contains_text(visit_url("manor3.php"), "chambera.gif")) {
			cli_execute("maximize mox -0.01 ml -tie -melee");
...
boolean bcascMacguffinPyramid() {
	...
	if (!contains_text(visit_url("questlog.php?which=1"),"A Pyramid Scheme") || contains_text(visit_url("questlog.php?which=1"),"found the little pyramid") || contains_text(visit_url("questlog.php?which=1"),"found the hidden buried pyramid")) {
		...
		while (item_amount($item[stone rose]) > 0) {
			...
			adventure(1, $location[desert (ultrahydrated)]);
		}
...
boolean bcascManor2ndFloor() {
	if (checkStage("manor2ndfloor")) return true;
	//Billiards Room
	while (item_amount($item[Spookyraven library key]) == 0) {
		while (item_amount($item[pool cue]) + equipped_amount($item[pool cue]) == 0) {
			bumAdv($location[Haunted Billiards Room], "", "", "1 pool cue", "Getting the Pool Cue");
		}
		
		print("BCC: Getting the Key", "purple");
		while (item_amount($item[Spookyraven library key]) == 0) {
			//Why are we not checking Moxie? Because we're doing this after the Boss Bat. 
			if (i_a("handful of hand chalk") > 0 || have_effect($effect[Chalky Hand]) > 0) {
				print("BCC: We have either a hand chalk or Chalky Hands already, so put the pool cue on...", "purple");
				if (i_a("handful of hand chalk") > 0 && have_effect($effect[Chalky Hand]) == 0) {
					use(1, $item[handful of hand chalk]);
				}
				cli_execute("goal clear");
				cli_execute("goal set 1 Spookyraven library key");
				cli_execute("maximize mox +equip pool cue");
...
boolean bcascPirateFledges() {
	...
	if (i_a("pirate fledges") == 0) {
		//I am arbitrarily (arrrrbitrarily?) assigning 110 buffed moxie to be the necessary amount to go into the barrr.
		levelMe(110, false);
		cli_execute("outfit swash");
		...
		if (index_of(visit_url("cove.php"), "cove3_3x1b.gif") == -1) {
			...
			if (index_of(visit_url("questlog.php?which=1"), "Caronch has offered to let you join his crew") > 0) {
				...
				if (i_a("Cap'm Caronch's Map") == 0)
					bumAdv($location[Barrrney's Barrr], "mox +outfit swash -ml", "", "1 Cap'm Caronch's Map", "Getting the Cap'm's Map", "", "consultBarrr");
			...
			if (i_a("Cap'm Caronch's nasty booty") > 0)
				bumAdv($location[Barrrney's Barrr], "mox +outfit swash -ml", "", "1 Orcish Frat House blueprints", "", "Getting the Blueprints", "consultBarrr");
			...
			if (i_a("Cap'm Caronch's dentures") > 0) {
				...
				while (available_amount($item[Cap'm Caronch's dentures]) > 0) adventure(1, $location[Barrrney's Barrr]);
			}
		...
		bumAdv($location[F'c'le], "mox +outfit swash -0.01 ml", "items", "1 pirate fledges", "Getting the Pirate Fledges, finally!", "+i");
	}
...
boolean bcascTavern() {
    ...
    setMood("");
	cli_execute("maximize mox -ml -melee");
    ...
    if (contains_text(visit_url("tavern.php?place=barkeep"), "ditzy stuff like that")) {
		checkStage("tavern", true);
		return true;
	}
}
...
void bcs3() {
	...
	//Tavern first. Safe mox is much lower than knoll, but still higher than 8.
	if (index_of(visit_url("questlog.php?which=1"), "a bit of a rat") > 0) {
		levelMe(20, false);	
		abort("New Tavern quest to be implemented!");
	}
...
void bcs9() {
	...
	if (i_a("giant pinky ring") == 0) cli_execute("leaflet");
...
void bcs12() {
	boolean doSideQuest(string name) {
		...
		int numMolyItems() {
			return item_amount($item[molybdenum hammer]) + item_amount($item[molybdenum crescent wrench]) + item_amount($item[molybdenum pliers]) + item_amount($item[molybdenum screwdriver]);
		}
		...
		switch (name) {
			...
			case "junkyard" :
				...
				while (get_property("currentJunkyardTool") != "") {
					bumAdv(to_location(get_property("currentJunkyardLocation")), "mox +DA +10DR -melee", "items", "1 "+get_property("currentJunkyardTool"), "Getting "+get_property("currentJunkyardTool")+"...", "", "consultJunkyard");
					visit_yossarian();
				}
			...
			case "nuns" :
				...
				cli_execute("maximize mox -ML -melee +outfit "+bcasc_warOutfit);
	...
	boolean killSide(int numDeadNeeded) {
		...
		if (bcasc_doWarAs == "frat") {
			...
			cli_execute("maximize moxie -ML, -melee, +outfit war hippy");
		} else {
			abort("adskd");
		}
	...
	if (index_of(visit_url("questlog.php?which=1"), "Make War, Not... Oh, Wait") > 0) {
		...
		if (bcasc_doWarAs == "hippy") {
			if (i_a("reinforced beaded headband") == 0 || i_a("bullet-proof corduroys") == 0 || i_a("round purple sunglasses") == 0) {
				...
				cli_execute("maximize mox -ML +outfit frat boy");
				...
				bumAdv($location[Wartime Hippy Camp], "mox -ML +outfit frat boy", "hebo", "1 reinforced beaded headband, 1 bullet-proof corduroys, 1 round purple sunglasses", "Getting the War Hippy Outfit", "", "consultHeBo");
			}
		} else if (bcasc_doWarAs == "frat") {
			if (i_a("beer helmet") == 0 || i_a("distressed denim pants") == 0 || i_a("bejeweled pledge pin") == 0) 
				bumAdv($location[Wartime Frat House], "mox -0.01ML +outfit hippy", "hebo", "1 beer helmet, 1 distressed denim pants, 1 bejeweled pledge pin", "Getting the Frat Warrior Outfit", "", "consultHeBo");
		} else {
		...
		if (!contains_text(visit_url("questlog.php?which=1"), "war between the hippies and frat boys started")) {
			if (bcasc_doWarAs == "hippy") {
				bumAdv($location[Wartime Frat House (Hippy Disguise)], "mox -0.01ML +outfit war hippy", "", "", "Starting the war by irritating the Frat Boys", "-");
			} else if (bcasc_doWarAs == "frat") {
				...
				bumAdv($location[Wartime Hippy Camp (Frat Disguise)], "mox -0.01ML +outfit frat war", "", "", "Starting the war by irritating the Hippies", "-");
			}
		}
	...
	if (contains_text(visit_url("main.php"), "lair.php")) {
			...
			foreach x in lairitems {
				if (contains_text(html, lairitems[x].gatename)) {
					...
					if (i_a(lairitems[x].a) > 0 || have_effect(to_effect(lairitems[x].effectname)) > 0) {
						...
						print("BCC: We do not have the item for that gate.", "purple");
						...
						if (contains_text(lairitems[x].a, "chewing gum") && cloversAvailable(true) > 0) {
							...
							cli_execute("use * large box; use * small box");
							
							setFamiliar("items");
							setMood("i+");
							while (bangPotionWeNeed() == "") {
								while (!identifyBangPotions()) {
		...
		void fightNS() {
			...
			if (bcasc_fightNS) {
				...
				for i from 1 to 3 {
					if (!contains_text(run_combat(), "You win the fight!")) {
